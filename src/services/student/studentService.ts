...
class StudentService {
  ...
  // TNEA Registration - Final Submit
  async submitApplication(registration: any): Promise<any> {
    const formData = new FormData();

    // Clone and strip file fields for JSON payload
    const payload = JSON.parse(JSON.stringify(registration));
    if (payload?.community_info) {
      payload.community_info.communityFile = undefined;
    }
    if (payload?.academic_details) {
      payload.academic_details.marksheetFiles = undefined;
    }
    if (payload?.marks) {
      payload.marks.combinedMarksheetFile = undefined;
    }
    if (payload?.declaration) {
      payload.declaration.signatureFile = undefined;
    }

    formData.append('data', JSON.stringify(payload));

    // Attach files if present
    const communityFile: File | null = registration?.community_info?.communityFile || null;
    if (communityFile) formData.append('community_certificate', communityFile);

    const marksheetFiles: File[] = registration?.academic_details?.marksheetFiles || [];
    marksheetFiles.forEach((file: File) => formData.append('marksheets[]', file));

    const combined: File | null = registration?.marks?.combinedMarksheetFile || null;
    if (combined) formData.append('combined_marksheet', combined);
